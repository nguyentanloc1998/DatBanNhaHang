...
public class ThemKhachHang extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/nhanvien/pages/customer/themkhachhang.jsp").forward(request, response);
}
	...
	//	System.out.println(checkusername(username));
		if(checkusername(username)) {
			if(pass.equals(pass1)) {	
				...
				acDAO.save(tk);
				response.sendRedirect(request.getContextPath()+"/XemDsKhachHang");
				return;
			}else {
